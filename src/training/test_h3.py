...
# --- 1. DÉFINITION DE LA DISTANCE HAVERSINE ---
...
def main():
    print("--- DÉMARRAGE DU TEST COMPARATIF AVEC RÉSOLUTION H3 (RÉSOLUTION 5) ---")
    ...
    # Feature Engineering temporel, distance et âge
    ...
    # Application de l'indexation Uber H3 (Résolution 5)
    print("Calcul des index Uber H3 (Résolution 5) pour l'acheteur et le marchand...")
    df["h3_client"] = [
        h3.latlng_to_cell(lat, lon, 5) for lat, lon in zip(df.lat, df.long)
    ]
    df["h3_merchant"] = [
        h3.latlng_to_cell(lat, lon, 5) for lat, lon in zip(df.merch_lat, df.merch_long)
    ]
    ...
    # Définition des 4 listes de colonnes avec H3
    feature_sets_h3 = {
        "FeatureSet_1_H3_Res5": [
            "merchant",
            "category",
            "amt",
            "first",
        ...
        "FeatureSet_2_No_Personal_IDs_H3_Res5": [
            "category",
            "amt",
            "gender",
            "city",
        ...
        "FeatureSet_3_No_State_City_H3_Res5": [
            "category",
            "amt",
            "gender",
            "city_pop",
        ...
        "FeatureSet_4_No_CityPop_H3_Res5": [
            "category",
            "amt",
            "gender",
            "distance_achat",
    ...
    for name, cols in feature_sets_h3.items():
        ...
        with mlflow.start_run(run_name=name):
            ...
            mlflow.log_param("lat_lon_representation", "uber_h3_res_5")
    ...
    json_path = os.path.join(script_dir, "metrics_h3_comparison.json")
    with open(json_path, "w") as f:
        json.dump(results, f, indent=4)
    print(f"\nComparatif H3 exporté en JSON : {json_path}")
