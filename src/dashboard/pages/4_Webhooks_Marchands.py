...
st.write("Historique en temps réel des alertes de fraudes envoyées aux serveurs des marchands partenaires (toutes les alertes des 24 dernières heures glissantes sont conservées dans Redis).")
st.markdown("---")
    ...
    for raw in alerts_raw:
        try:
            ...
            alerts_list.append({
                ...
                "Prédiction Modèle": "🚨 Suspect" if int(data.get("prediction", 0)) == 1 else "✅ Sain",
...
if alerts_list:
    ...
    st.metric(f"Alertes affichées pour {selected_merchant}", f"{total_alerts} / {len(df_alerts)}")
else:
    st.info("Aucune alerte de fraude suspecte identifiée sur les dernières 24 heures glissantes (Redis et base de données vides).")
