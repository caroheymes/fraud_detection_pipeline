...
with st.expander("🛠️ Espace de test & Intégration API Marchand (cURL)"):
    ...
    st.write(
        "Voici un exemple de commande **cURL** incluant un **Auth-Token de sécurité (X-Merchant-Token)** et le **numéro de carte chiffré par SHA-256** :"
    )
    ...
    mock_request = {"transaction_id": "simulated_tx_web_999"}
    request_str = json.dumps(mock_request, indent=2)

    st.code(
        f"""curl -X POST "https://fraud-detection.ngrok.app/mock-merchant-webhook" \\
